...
public class TrackCompareManager {
	...
	public Comparator<Pair<String, ? extends Collection<? extends PrimaryDocument>>> getComparator(String s) {
		s = s.toLowerCase();
		...
		while (i >= 0){
			...
			if (comparators.containsKey("" + c[i])){
				...
				Comparator<Pair<String, ? extends Collection<? extends PrimaryDocument>>> next = 
					comparators.get("" + c[i]);
				
				int j = i - 1;
				while (j >= 0 && !comparators.containsKey("" + c[j]) && !(c[j] == '-')){
					j--;
				}
				...
				Comparator<Pair<String, ? extends Collection<? extends PrimaryDocument>>> chained = ComparatorUtils.chainedComparator(next, result);
				result = chained;
			}
			i--;
		}
	...
	public abstract class TrackComparator implements
	...
	public class NameComparator extends TrackComparator {
		@SuppressWarnings("unchecked")
		public int compare(Pair<String, ? extends Collection<? extends PrimaryDocument>> o1,
	...
	public interface ToLong {
		public long toLong(Pair<String, ? extends Collection<? extends PrimaryDocument>> pair);
	}
	...
	public class StartComparator extends NumericalComparator {
		public StartComparator() {
			super(new ToLong() {
				public long toLong(Pair<String, ? extends Collection<? extends PrimaryDocument>> o1) {
		...
		public MedianComparator() {
			super(new ToLong() {
				...
				public long toLong(Pair<String, ? extends Collection<? extends PrimaryDocument>> o1) {
					...
					ArrayList<PrimaryDocument> a = new ArrayList<PrimaryDocument>(o1.v.size());

					for (PrimaryDocument pd : o1.v) {
						if (PrimaryDocumentDot.getDate(pd) != null)
							a.add(pd);
					}
					...
					Collections.sort(a, new Comparator<PrimaryDocument>() {
						public int compare(PrimaryDocument o1, PrimaryDocument o2) {
							return c.compare(PrimaryDocumentDot.getDate(o1), PrimaryDocumentDot
								.getDate(o2));
						};
					});

					if ((a.size() & 1) == 0) {
						return (PrimaryDocumentDot.getDate(a.get(a.size() / 2 - 1)).getMillis() + PrimaryDocumentDot
							.getDate(a.get(a.size() / 2)).getMillis()) / 2;
					} else {
						return PrimaryDocumentDot.getDate(a.get(a.size() / 2)).getMillis();
					}
				}
			});
		}
