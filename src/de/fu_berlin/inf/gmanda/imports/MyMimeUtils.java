...
public class MyMimeUtils {
	...
	public static Folder getFolder(final IProgress progress, File mboxFile)
	...
	public static Folder getFolder(File mboxFile, StatusListener listener)
			throws MessagingException {

		Session session = Session.getDefaultInstance(new Properties());
		MboxStore store = (MboxStore) session.getStore(new URLName("mbox:"
				+ mboxFile.getAbsolutePath()));
		store.connect();
		if (listener != null)
	...
	protected static String getFromInternal(Message m)
			throws MessagingException {
		try {
	...
	public static String getRecipients(Message m, Message.RecipientType type)
			throws MessagingException {
		try {
	...
	public static String toString(Multipart mp) throws MessagingException,
			IOException {
		...
		if (StringUtils.indexOf(mp.getContentType(), "multipart/alternative") == 0) {
			...
			for (int i = 0; i < mp.getCount(); i++) {
				if (StringUtils.indexOf(mp.getBodyPart(i).getContentType(),
						"text/plain") == 0) {
					bestChoice = i;
				}
			}
	...
	public static String toString(Part part) throws MessagingException,
			IOException {
