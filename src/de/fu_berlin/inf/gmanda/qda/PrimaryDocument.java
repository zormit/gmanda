...
public class PrimaryDocument implements Comparable<PrimaryDocument>, Codeable {
	...
	public static String guessListFromFileName(String filename){
		Matcher m = Pattern.compile("(comp\\.[a-z.]+?)\\.(?=\\d|mbox)").matcher(filename);
	...
	/**
	 * This is operation is potentially long running as it will refetch
	 * from the web, if the file is not available
	 * 
	 * @param gmane
	 * @return
	 */
	...
	public static TreeWalker<PrimaryDocument> getTreeWalker(PrimaryDocument primaryDocument) {
	...
	public void refetchRecursive(IProgress progress, GmaneMboxFetcher gmaneMboxFetcher) {
		...
		try {
			...
			if (pdmap.size() > 0) {
				try {
					List<Message> messages = gmaneMboxFetcher.getMails(progress.getSub(50), list, min, max + 1);
	...
	public void updateFromMessage(Message m, int number, String list) throws Exception {
	...
	public void refetch(GmaneMboxFetcher gmaneMboxFetcher) {
		...
		Message m;
		try {
			m = gmaneMboxFetcher.getSingleMail(new NullProgressMonitor(), list, number);
	...
	public static Comparator<PrimaryDocument> getProjectSensitiveComparator(final Project project){
		return new Comparator<PrimaryDocument>(){
			public int compare(PrimaryDocument o1, PrimaryDocument o2) {
				...
				return Integer.signum(project.getPrimaryDocuments().indexOf(list1) -
	...
	public static List<PrimaryDocument> filterChildren(List<PrimaryDocument> list) {
		...
		List<PrimaryDocument> result = new ArrayList<PrimaryDocument>(list.size());
