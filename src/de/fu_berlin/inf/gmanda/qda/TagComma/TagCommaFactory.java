...
public class TagCommaFactory {
	...
	static public class TagCommaString extends AbstractCodedString {
		...
		public class TagCodedCode extends AbstractCode {
			...
			public String format(int indent, int width) {
				...
				// If there is more than one newline at the beginning, we want
				// to
				// keep one of it.
				...
				if (oldValue != null)
					assert sb.toString().replaceAll("\\s", "").equals(
						oldValue.replaceAll("\\s", "")) : sb.toString() + " \n\n" + oldValue;
			...
			public List<Code> getProperties() {
				...
				Pattern p = Pattern
					.compile("(memo|date|desc|summary|milestone|def|value|vdef|quote|ref|title|cause)\\s*:\\s*");
				...
				while (m.find()) {
					...
					if (currentValue.length() > 0) {
						result.add(new TagCodedCode(lastKey
							+ "=\""
							+ org.apache.commons.lang.StringUtils.strip(currentValue.toString(),
								", \n\r\f\t") + "\""));
			...
			public Code renameTag(String fromRename, String toRename) {
				throw new UnsupportedOperationException(
					"TagCommaSyntax is no longer supported except for reading");
			}
		}
		...
		public CodedString rename(String from, String to) {
			throw new UnsupportedOperationException(
				"TagComma is deprecated and this operation is no longer supported");
		}
