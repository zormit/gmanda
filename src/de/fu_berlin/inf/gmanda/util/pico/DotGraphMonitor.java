package de.fu_berlin.inf.gmanda.util.pico;
...
public final class DotGraphMonitor extends AbstractComponentMonitor implements
	...
	public String getClassDependencyGraph() {
		...
		for (Pair<String, List<Class<?>>> p : Pair.partition(
			...
			if (color == null) {
				System.out.println("No color found for Module " + p.p);
				color = "black";
			}
	...
	public static final class ModuleFunction implements
		...
		public String apply(Class<?> u) {
			...
			if (c == null) {
				System.out
						.println("Injected component with no @Component annotation: "
								+ u.getSimpleName());
	...
	@Override
	public void stop() {
		File f = new File("GmanDA-architecture.dot");
		System.out.println("Saving Saros architecture diagram dot file: "
				+ f.getAbsolutePath());
		try {
			save(f);
		} catch (IOException e) {
